#-----------------------------------------------------------#
# Simulation Study building on GDP2
#-----------------------------------------------------------#
...
for (r in 1:rep){
  ...
  # sample the under-reported observations
  sample(Y[Y>minimum],share_above*n_edges)
  
  # real underreported
  flag<-which(Y%in% sample(Y[Y>minimum],share_above*n_edges))
  ...
  while(diff>stop){
    ...
    plot(Y_real~result$Y_imputed,col=col)
    abline(a=0,b=1)
    # M-Step
...
int_positive<-c()
  ...
  share<-c(share,  sum(probis[flag]>opt_cutoff))
  ...
  A1<-n_edges*share_above
  A2<-0.75*n_edges-A1
  
  
  
  A3<-sum(probis[flag2]>opt_cutoff)
  A4<-sum(probis[flag]>opt_cutoff)
  A5<-A1-A4
  A6<-A2-A3
  TP<-c(TP,A4 )
  FP<-c(FP, A3 )
...
save.image("5) Simulation Study/data_dgp2.RData")
